...
for the available entries  that a matrix with one column will be formed. 
In this case it will be len(y)   rows. np.ravel()  is needed because scaling
produces 2d array and we still need to use regressor.fit() . So your y still must be 
1d array, and np.ravel() flattens any array into 1d.'''
